...
describe('Link', () => {
  ...
  it('renders the passed in children', () => {
    const result = shallow(
      <Link children="https://www.draft-js-plugins.com/" />
    );
  ...
  it('uses the decoratedText prop as href', () => {
    const result = shallow(
      <Link decoratedText="https://www.draft-js-plugins.com/" />
    );
    expect(result)
      .to.have.prop('href')
      .to.contain('https://www.draft-js-plugins.com/');
  });
  ...
  it('applies http prefix when none is supplied', () => {
    ...
    expect(result)
      .to.have.prop('href')
      .to.contain('http://draft-js-plugins.com/');
  });

  it('does not apply a prefix when one is already supplied', () => {
    const result = shallow(
      <Link decoratedText="ftp://draft-js-plugins.com/" />
    );
    expect(result)
      .to.have.prop('href')
      .to.contain('ftp://draft-js-plugins.com/');
  });
  ...
  it('generates correct href to localhost with port', () => {
    ...
    expect(result)
      .to.have.prop('href')
      .to.contain('http://localhost:8000');
  });
  ...
  it('generates mailto href when supplied with email', () => {
    ...
    expect(result)
      .to.have.prop('href')
      .to.contain('mailto:name@example.com');
  });

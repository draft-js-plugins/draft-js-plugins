...
export default function createTextAlignmentButton({
  ...
  return function InlineStyleButton(props) {
    const toggleStyle = (event: MouseEvent): void => {
      ...
      const removeStyles = alignmentStyles.filter(style => style !== alignment);
      ...
      if (selectionIsBackward) {
        ...
        const modifiedContent = Modifier.applyInlineStyle(currentContent, selectWholeBlocks, alignment);
        const finalContent = removeStyles.reduce((content, style) => Modifier.removeInlineStyle(content, selectWholeBlocks, style), modifiedContent);
        const nextEditorState = EditorState.push(currentEditorState, finalContent, 'change-inline-style');
